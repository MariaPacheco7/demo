...
public class ProductRepositoryImpl implements ProductRepository {
	...
	public Optional<Product> findById(Integer id) {
		try {
			//Product product = jdbcTemplate.queryForObject( sql: "SELECT * FROM products WHERE id = ?",
			Product product = jdbcTemplate.queryForObject( "SELECT * FROM products WHERE id = ?",
					...
					(rs, rowNum) -> {
						...
						//p.setName(rs.getString( columnLabel: "name"));
						p.setName(rs.getString("name"));
						//p.setQuantity(rs.getInt( columnLable: "quantity"));
						p.setQuantity(rs.getInt("quantity"));
						//p.setVersion(rs.getInt( columnLabel: "version"));
	...
	public List<Product> findAll() {
		//return jdbcTemplate.query( sql: "SELECT * FROM products",
		return jdbcTemplate.query("SELECT * FROM products",
				(rs, rowNumber) -> {
					...
					//product.setVersion(rs.getInt( columnLabel: "version"));
